use crate::{input_utils::verify_parenthood, sql::get_fk_match_config};
...
pub fn update_all(
    ...
    if input.contains_key(&schema.pk) {
        return Err(anyhow!(
            "'{}' cannot be updated. It's \"{}\"'s primary key. The attempted update is {:?}",
            schema.pk,
            table,
            input
        ));
...
pub fn update_children_of(
    ...
    let combined_q_config = get_fk_match_config(parent_info, where_config_opt)?;
    update_all(
        conn,
        schema_family,
        child_table,
