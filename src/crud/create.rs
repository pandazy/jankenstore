...
use super::verify::{get_verified_write_inputs, verify_table_name, VerifyConfig};
...
/// * `verification_options` - the options for verification, if None, no verification is performed, see [VerifyConfig] for more details
pub fn i_one(
    conn: &Connection,
    table_name: &str,
    ...
    let verified_input = get_verified_write_inputs(true, table_name, input, verification_options)?;
    let mut params = vec![];
    let mut columns = vec![];
    let mut values = vec![];
