    ...
    let verify_required_field_with_value = |field: &str, value: &types::Value| -> Result<()> {
        if schema.required_fields.contains(field) && is_empty(value) {
            return Err(anyhow!(
                ...
                if default_if_absent {
                    format!(
                        "Default value of '{}': {:?}\n{}{}",
                        field,
                        schema.defaults.get(field),
                        "Also, because `default_if_absent` is true, ",
                        "absent fields, if any, will be assigned with corresponding default values before being checked"
                    )
                } else {
                    "".to_string()
                }
...
pub fn json_to_val_by_schema(
    ...
    let the_type = schema.types.get(col_name).unwrap_or(&types::Type::Null);
...
pub fn verify_column_val(
    ...
    let schema = schema_family.try_get_schema(table)?;
    let the_type = schema.types.get(col_name).unwrap_or(&types::Type::Null);
    if !col_val.data_type().eq(the_type) {
        return Err(anyhow!(
